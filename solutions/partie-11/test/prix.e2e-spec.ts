...
describe('Prix (e2e)', () => {
  ...
  beforeAll(async () => {
    // 11.9 : configurerApp lit la configuration (l'adresse de Redis, pour RedisIoAdapter) : ConfigModule, sans .env.
    const module = await Test.createTestingModule({ imports: [ConfigModule.forRoot({ isGlobal: true, ignoreEnvFile: true }), PrixModule] }).compile();
