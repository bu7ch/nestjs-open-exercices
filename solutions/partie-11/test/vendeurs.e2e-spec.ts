...
describe('Vendeurs (e2e, avec la vraie base)', () => {
  ...
  it('crée, relit, renomme puis supprime un vendeur', async () => {
    ...
    expect(creation.body.data).toMatchObject({ id: 1, nom: 'Atelier du Bois' });

    const lecture = await http().get('/api/vendeurs/1').set('Authorization', bearer).expect(200);
    expect(lecture.body.data).toMatchObject({ id: 1, nom: 'Atelier du Bois' });
    ...
    expect(renomme.body.data.nom).toBe('Atelier du Chêne');
