...
4. Vérifie-les :
   ...
   npm run test:partie-10  # les exercices de la partie 10 (mise en production, base ET Redis démarrés)
   npm run test:partie-11  # les exercices de la partie 11 (temps réel, files de tâches, webhooks ; base ET Redis)
...
- Ils **ne lisent jamais ton `.env`** : chaque test fournit lui-même ses variables (`PORT`, `NOMBRE_MAX_PRODUITS`, `DB_*`, `JWT_SECRET`, `REDIS_*`…). Garde ton `.env` pour `npm run start:dev`.
...
À partir de la partie 10, le même `compose.yaml` démarre aussi **Redis** (`redis:7-alpine`, port 6379) : les tests de la partie 10 s'y connectent avec `REDIS_HOST=localhost` et `REDIS_PORT=6379` (fixés dans `vitest.config.ts`, surchargeables : `REDIS_PORT=56379 npm test`). Si ton conteneur de la base tourne déjà, `docker compose up -d redis` suffit.
...
Le dossier `solutions/partie-N/src` contient une solution de la partie N. Elle sert à deux choses : prouver que les exercices sont faisables (GitHub Actions vérifie que chaque solution passe ses tests), et te dépanner **après** avoir essayé. Chaque solution est une copie complète de `src/` : la solution de la partie 4 reprend celle de la partie 3. À partir de la partie 6, elle contient aussi ce que tu écris hors de `src/` : `test/`, `.env.test`, `vitest.config.unit.ts` et `vitest.config.e2e.ts` ; en partie 10, `Dockerfile`, `.dockerignore`, `compose.yaml`, `compose.prod.yaml` et `.github/workflows/ci.yml`. Depuis la partie 8, les variables de test de la solution sont dans `vitest.config.e2e.ts` (et non dans un `.env.test`).
...
| 10 · Mise en production | 10.1 (validation de tout l'environnement, message d'une ligne), 10.2 (refus de démarrer, code 1 ; `/docs` en `404` en production), 10.3 (`data-source.ts` et `migration:show` sans `.env`), 10.4 à 10.6 (Dockerfile multi-étapes, `USER` non root, `CMD` sur `node`, `.dockerignore`, ordre des couches, aucun secret dans `ENV`/`ARG`), 10.7 (`compose.yaml` : services, healthchecks, `depends_on` et conditions, variables), 10.8 (validation Redis, `RedisThrottlerStorage`, compteurs qui survivent au redémarrage), 10.10 (`/sante` et `/sante/pret`, 503 base ou Redis coupés, `@SkipThrottle`, exclusion `sante{/*reste}`), 10.11 (`creerLogger`, paramètres du middleware, journaux JSON de l'application compilée), 10.12 (`GET /api/demo/longue`, `SIGTERM` qui laisse finir la requête, `stop_grace_period`), 10.13 et 10.14 (le workflow : services, build, migrations, job `image`, GHCR), 10.15 (`compose.prod.yaml`, `trust proxy`) ; **tes tests** des 10.1, 10.10, 10.11 et 10.15 jugés par mutation, ton fichier des abus relancé deux fois (10.8) | — | voir la section « Partie 10 » : les mesures (tailles, temps, `CACHED`, contexte), `Dockerfile.fuite` et `docker history`, tout ce qui se lance avec `docker compose` (10.7 à 10.12), 10.9, le push, l'onglet Actions, actionlint et la migration fautive (10.13), GHCR (10.14), le lancement de `compose.prod.yaml` (10.15) |
| 11 · Aller plus loin | 11.1 (`vendeur:suivre`, salle `vendeur:<id>`, accusé `{ ok: true }`, `commande:creee` au seul vendeur concerné), 11.2 (DTO, `ErreursWsFilter` : erreurs de validation et vendeur inconnu dans l'accusé), 11.3 (le guard global laisse passer les gateways ; `cors` du gateway, `mechant.example` refusé), 11.4 (`Jeton manquant`, `Jeton invalide ou expiré`, propriétaire ou admin, rien pour les autres), 11.5 (chacun ses lignes, `Commande` → acheteur rempli depuis le jeton et sa migration, salle `compte:<id>`, `commande:statut` à l'acheteur seul), 11.6 (rattrapage dans l'accusé, `disconnect()`/`connect()` et vraie coupure), 11.7 (job différé `expiration` / `expiration-<id>` au délai `DELAI_PAIEMENT_MS`, `annulee` et `ignoré`, `commande:statut`, pour de vrai à 200 ms), 11.8 (`marquerPayee` : `payee`, job retiré de Redis), 11.9 (le job survit au redémarrage ; deux instances et `RedisIoAdapter`) | 11.10 à 11.12 (import CSV : `202`, `GET /api/imports/:id`, réessais sans doublon, backoff, `UnrecoverableError`, `jobId` par empreinte), 11.13 à 11.15 (webhook signé : corps brut, `rawBody` dans `main.ts`, `400` des signatures fausses, absentes, vieilles ou d'un corps modifié, doublon, livraisons simultanées, panne puis rejeu, commande inconnue) | voir la section « Partie 11 » : les interfaces typées (11.1), les essais « casser exprès » et ce qu'on y note (11.2, 11.3, 11.7, 11.8, 11.9, 11.13, 11.14), **tes propres tests** (non jugés), les phrases à écrire (11.12, 11.15) |
...
## Partie 10 : mise en production

`@nestjs/terminus`, `ioredis` et `yaml` (pour lire tes fichiers YAML dans les tests) sont déjà dans le `package.json` : `npm install` suffit. Il te faut **la base et Redis** : `docker compose up -d` (le `compose.yaml` du dépôt démarre maintenant `db` et `redis`). Une fois ton `compose.yaml` complété au 10.7, Compose lit ton `.env` pour `${JWT_SECRET:?…}` et les autres variables obligatoires, même pour `docker compose up -d db redis` : garde-les dans ton `.env`.

**Trois façons de vérifier tes exercices.**

1. **Ton application, démarrée par les tests** comme dans les parties 7 à 9 (leurs secrets, la base de test, et le Redis des tests) : la validation d'environnement (appelée directement), `data-source.ts` chargé sans `.env`, `RedisThrottlerStorage` sur le vrai Redis, les compteurs qui survivent à un redémarrage, `/sante/pret` avec la base fermée (`destroy()`, comme le cours) puis Redis déconnecté, `trust proxy` (`X-Forwarded-For`), `creerLogger` et les paramètres du journal.
2. **Ton application compilée**, lancée comme en production : `nest build` dans une copie de ton projet (ton `dist/` n'est pas touché), puis `node dist/main.js` dans un processus à part, avec `NODE_ENV=production` et **aucun fichier `.env`**. Les tests y vérifient le refus de démarrer (code `1`, le message qui nomme les variables, `NODE_ENV=prod` refusé), `/docs` en `404`, les journaux JSON (`params.requeteId`…), `npx typeorm migration:show -d dist/data-source.js` sans `.env`, et l'arrêt propre : un `SIGTERM` envoyé pendant `GET /api/demo/longue` doit laisser la requête finir en `200`. Hors conteneur, NestJS se renvoie le signal une fois les requêtes finies : le processus se termine par ce `SIGTERM` (ou le code `0`), c'est normal. Si ton projet ne compile pas, ces tests le disent d'abord.
3. **Tes fichiers de déploiement**, lus sans rien exécuter : `Dockerfile`, `.dockerignore`, `compose.yaml`, `compose.prod.yaml` et `.github/workflows/*.yml`. Les noms de tes services sont libres : la base est le service dont l'image est PostgreSQL, Redis celui dont l'image est Redis, les migrations celui dont la commande contient `migration:run`, l'API le service restant. Pour le workflow, le « job des tests » est celui qui lance `npm test` (ou `npm run test:partie-…`).

**Docker, pour de vrai (facultatif).** `docker.spec.ts` construit ton image (`docker build`, une à trois minutes), vérifie `id` (pas root), `ls -a` (seulement `dist`, `node_modules`, `package.json`), le code `1` sans variables, `docker image inspect` (seulement `NODE_ENV=production`), la présence de la CLI de TypeORM et l'absence de celle de NestJS, puis valide tes fichiers avec `docker compose config` (sans rien lancer). L'image de test est supprimée à la fin, rien n'est poussé. Ces tests sont **ignorés** (*skipped*) si `docker info` ne répond pas, dans une CI (variables `CI` ou `GITHUB_ACTIONS` : ils ne tournent jamais sur GitHub), ou avec `TESTS_DOCKER=0` ; `TESTS_DOCKER=1` les force.

**Tes tests sont jugés**, comme en parties 6, 8 et 9 : ton test de `validerEnvironnement` (10.1 : conversion de `NOMBRE_MAX_PRODUITS`, `DB_HOST` obligatoire, `NODE_ENV (isIn)`), ton `src/journal.spec.ts` (10.11 : texte en production, paramètres ou contexte perdus, `debug` écrit en production, JSON en développement), tes fichiers e2e qui parlent de `/sante/pret` (10.10 : prêt toujours à `200`, `503` sans le rapport, vivant qui vérifie la base, `X-Request-Id` sur `/sante/pret`) et de `X-Forwarded-For` (10.15 : `trust proxy` coupé). Ton fichier des abus (tout fichier e2e qui écrit `THROTTLE_ACTIF = 'true'`) est lancé **deux fois de suite** : sans `flushdb()` avant chaque test, la seconde fois échoue (10.8). `REDIS_HOST` et `REDIS_PORT` sont imposés (comme `DB_*`) ; le reste vient de ton `.env.test`, ou est fourni s'il y manque.

**Redis est vidé.** Comme le `beforeEach` du cours, les tests qui réactivent la limitation vident le Redis des tests (`FLUSHDB`) : n'y garde rien d'important. Le test des abus de la partie 7 le vide aussi, s'il répond (sinon, les compteurs étaient encore en mémoire).

**Ce qui reste à faire toi-même.** Retirer `reflect-metadata` et noter l'erreur (10.1) ; la version sans `@IsIn` et `curl -i` (10.2) ; l'erreur `ENOENT` avant la correction (10.3) ; `docker build`, les tailles de `docker images` et la version naïve (10.4) ; `CACHED`, les temps et la taille du contexte (10.5) ; `Dockerfile.fuite`, `docker history`, `docker rmi` et `npm run migration:run` dans l'image (10.6) ; tout ce qui se lance avec `docker compose` : `ps`, `\dt`, `No migrations are pending` (10.7), le `429` après `docker compose restart api` (10.8), les trois pannes et l'écart d'heure (10.9), l'état `healthy`/`unhealthy` (10.10), `docker compose logs api | grep` (10.11), `docker compose stop`, `-t 2` et les codes de sortie (10.12) ; la correction du test du 8.5 (10.11, non jugée) ; le push, l'onglet Actions, actionlint et la migration fautive (10.13) ; l'onglet Packages, `docker pull`, `id` et la pull request (10.14) ; `pull`/`up -d` de `compose.prod.yaml` et `curl http://127.0.0.1:3000/sante/pret` (10.15).

**La CI de ton fork.** Le workflow du dépôt démarre maintenant un Redis dans ses deux jobs (`REDIS_HOST: localhost`). Au 10.13, tu y ajoutes `npm run build` et les migrations avant les tests. Au 10.14, ton job `image` dépend (`needs`) du job des tests : or `npm test` lance **toutes** les parties, dont celles qui ne peuvent plus passer sur ton code final, et le job `exercices` a `continue-on-error: true`. La solution de la partie 10 lance donc `npm run test:partie-10` dans ce job, sans `continue-on-error`, pour que `needs` ait un sens. Elle ajoute aussi la migration de `ProfilClerk` (bonus 7.25), que `migration:generate --check` réclame : les tests e2e créaient la table avec `synchronize`, aucune migration ne l'avait jamais créée.

**Ce qui ne passe plus.** Rien de nouveau pour les parties 8 et 9 : leurs tests passent sur la solution de la partie 10. Ceux des parties 3 à 7 qui tombaient déjà sur la solution de la partie 9 tombent toujours, et seulement eux. La solution de la partie 10 n'est vérifiée que par les tests de la partie 10 ; ses propres tests (`solutions/partie-10/test/`, dont le fichier de référence du contrat, mis à jour avec `/sante/pret` et `/api/demo/longue`) passent aussi.

## Partie 11 : aller plus loin

Les paquets de la partie 11 sont déjà dans le `package.json` : `@nestjs/websockets`, `@nestjs/platform-socket.io`, `socket.io-client` (pour les tests), `@nestjs/bullmq`, `bullmq`, `@socket.io/redis-adapter` et, pour le bonus e, `stripe`. `npm install` suffit (npm peut afficher un avertissement `allow-scripts` pour `msgpackr-extract`, un accélérateur facultatif de BullMQ : sans lui, BullMQ fonctionne quand même). Il te faut **la base et Redis** (`docker compose up -d`), comme en partie 10.

**Ce que les tests fournissent.** Comme depuis la partie 7 : leurs secrets, des comptes inscrits et connectés par tes routes, passés `vendeur` ou `admin` en SQL, des boutiques, produits et variantes posés avec tes entités, des commandes passées par `POST /api/commandes` (avec le jeton de l'acheteur). En plus : `DELAI_PAIEMENT_MS=3600000` (aucune commande n'expire pendant un test, sauf dans ceux qui fixent 200 ms, 5 s ou 0), et `WEBHOOK_SECRET`, le secret avec lequel ils signent leurs livraisons. Ton application est créée avec `rawBody: true`, comme le cours le fait répéter aux tests (11.13), puis écoute sur un **port libre** : de vrais clients `socket.io-client` s'y connectent, en WebSocket, avec `io(url, { auth: { jeton } })` (le jeton est aussi dans `auth.token` et l'en-tête `Authorization`, si tu l'y lis). Ton gateway doit donc s'installer sur le serveur de l'API (`@WebSocketGateway()` sans numéro de port ; un `namespace` est accepté), et `RedisIoAdapter` se brancher dans `configurerApp` (11.9), où les tests le trouvent.

**Des attentes, jamais de délai au hasard.** Un événement attendu a 3 à 5 secondes pour arriver ; au-delà, le message dit lequel manque et pourquoi. Pour vérifier qu'un client ne reçoit **rien**, le test fait envoyer par ton serveur un dernier message à ce client, et attend qu'il arrive : socket.io livrant dans l'ordre, tout ce que ton code a émis avant est déjà là (c'est l'aller-retour du cours, fait côté serveur). Les jobs sont attendus dans Redis (`QueueEvents`), quel que soit le worker ou l'instance qui les traite. Tes files sont retrouvées dans ton application, **quels que soient leurs noms**, et vidées (`obliterate`) avant chaque test : n'y garde rien. Pour exécuter une expiration sans attendre une heure, le test avance le job (`job.promote()`).

**Le redémarrage (11.9).** En test, ton application vide la base à chaque démarrage (`dropSchema`, 6.12) : la commande de l'expiration disparaîtrait avec elle, et le job renverrait `'ignoré'`. Le test remet donc en base la commande et son acheteur, tels qu'ils étaient, après le démarrage de la nouvelle application et avant l'heure du job. Il vérifie d'abord, l'application arrêtée, que le job attend toujours dans Redis. Pour les deux instances, les deux applications tournent dans le même processus ; le worker de la première est mis en pause (`worker.pause()`).

**Ce qui est libre.** La forme de `produit` dans `commande:creee` (le nom, l'identifiant, ou un objet qui porte l'un ou l'autre) ; les éléments de l'accusé du 11.6 (`commandeId` ou `id`, et leurs `lignes`) ; les noms de tes files et de tes fichiers ; le nom de la relation `Commande` → `Compte` (11.5) ; le champ qui porte la raison d'un import échoué (11.11 : le test cherche `job.failedReason` dans la réponse) ; la réponse à une commande inconnue (11.15 : un `200` ou un `4xx`, jamais un `500`). Les réponses HTTP peuvent être enveloppées (`{ data }`, 8.13) ou non.

**Les bonus.** Les tests des files de tâches (`bonus-imports.spec.ts`) sont ignorés (*skipped*) tant qu'aucune route de `src/` ne contient `imports` ; ceux des webhooks (`bonus-webhooks.spec.ts`), tant qu'aucun fichier n'importe `stripe`. Pour faire échouer **une** écriture (le troisième produit du 11.11, le paiement du 11.15), le test pose une panne dans PostgreSQL lui-même (un trigger, retiré ensuite) : ton code n'est pas touché, quelle que soit la façon dont il écrit (repository, transaction, `vi.spyOn` du cours). Aucun compte Stripe : les livraisons sont signées par le test avec `generateTestHeaderString`, dont une écrite avec des espaces (seul le corps **brut** a la bonne signature). `main.ts` est lancé une fois, pour vérifier son `rawBody: true`.

**Ce qui reste à faire toi-même.** Les interfaces typées (11.1 : rien ne se vérifie à l'exécution) ; l'essai sans filtre, l'erreur du test et le contenu de l'événement `exception` (11.2) ; retirer la ligne `getType() !== 'http'`, puis l'option `cors`, et noter ce qui se passe (11.3) ; retirer le filtrage des lignes et voir tomber **ton** test (11.5) ; l'identifiant `expiration:12` et son erreur (11.7) ; ce que répond `file.remove(…)` pour un identifiant inconnu, et pourquoi `getJob` (11.8) ; l'erreur de ton test sans adaptateur (11.9) ; ta phrase sur `removeOnComplete` (11.12) ; le message sans `rawBody`, le statut de la seconde livraison sans dédoublonnage (11.13, 11.14) ; ta phrase sur la commande inconnue (11.15). **Tes propres tests** (le cours en demande à chaque exercice) ne sont pas jugés dans cette partie : écris-les quand même, ceux de la solution (`solutions/partie-11/test/`) peuvent t'inspirer.

**Le guard global.** Le correctif de la section a (`if (contexte.getType() !== 'http') return true;` en tête de ton `AuthGuard`) est indispensable : sans lui, chaque événement du gateway plante dans le guard. Il a un effet de bord à connaître : une requête GraphQL (bonus de la partie 5) n'est pas `http` non plus (`getType()` vaut `'graphql'`) ; elle passe désormais le guard **sans jeton**. La solution garde ce comportement du cours ; si tu gardes le bonus GraphQL, traite ce contexte à part.

**Ce qui ne passe plus.** Rien de nouveau : sur la solution de la partie 11, les tests des parties 8, 9 et 10 passent tous, et ceux des parties 3 à 7 qui tombaient déjà sur la solution de la partie 10 tombent toujours, et seulement eux. Trois pièges, pour ton code :

- si ta validation d'environnement **exige** `DELAI_PAIEMENT_MS` ou `WEBHOOK_SECRET`, les tests du 10.1, qui l'appellent avec un environnement complet… de la partie 10, tombent. Donne une valeur par défaut au délai, et lis le secret avec `getOrThrow` au moment de s'en servir, comme le cours (les autres tests fournissent ces deux variables) ;
- `configurerApp` lit maintenant la configuration (l'adresse de Redis, pour `RedisIoAdapter`) : un test e2e qui monte un petit module sans `ConfigModule` (le `test/prix.e2e-spec.ts` de la partie 6) ne peut plus l'appeler. Ajoute `ConfigModule.forRoot({ isGlobal: true, ignoreEnvFile: true })` à ce module, comme la solution ;
- l'adaptateur du cours ferme ses connexions Redis avec `quit()`, même quand elles sont encore en train de s'ouvrir (une application arrêtée aussitôt démarrée, ce que font beaucoup de tests) : les abonnements en attente sont rejetés (`Unhandled Rejection: Connection is closed.`), et Vitest compte ces erreurs contre tes tests, y compris ceux que jugent les parties 9 et 10. La solution attend que chaque connexion ait fini de s'ouvrir avant de la fermer (`redis-io.adapter.ts`) ; les tests de la partie 11, eux, attendent que l'adaptateur soit prêt avant de continuer.

La solution de la partie 11 n'est vérifiée que par les tests de la partie 11, et son workflow lance `npm run test:partie-11`. Ses propres tests (`solutions/partie-11/test/`) passent aussi : le fichier de référence du contrat a été mis à jour avec les routes d'import (le webhook, appelé par le prestataire, est exclu de la documentation avec `@ApiExcludeController()`). Deux écarts avec le code du cours, signalés en commentaire : `marquerPayee(id, base?)` accepte le gestionnaire de la transaction du webhook (11.14), pour que le paiement et la note de l'événement soient annulés ensemble ; et la fermeture de l'adaptateur, ci-dessus. Dans ses tests, `DELAI_PAIEMENT_MS` est lu une fois, au chargement d'`AppModule` : les tests des minuteurs avancent donc les jobs avec `job.promote()` au lieu de changer le délai.
